...
public partial class A2_Reports_CRMPerformanceReport : CustomBasePage, IRequiresSessionState
{
    ...
    private void BindData()
    {
        if (connection.State != ConnectionState.Open)
            connection.Open();
        SqlCommand selectCommand = new SqlCommand("XRec_SelectProductivityReportDataStatusWise", connection);
    ...
    protected void rptDeparments_OnItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        try
        {
            ...
            if (defaultViewMain != null)
                if (defaultViewMain.Table.Rows[0]["TotalScheduleCount"].ToString() == "0")
    ...
    {
        DataTable dataTable = new DataTable();
        SqlCommand selectCommand = new SqlCommand("XRec_SelectProductivityReportDataStatusWiseDetail", connection);
        selectCommand.Parameters.AddWithValue("@DateFrom", postedFromDate.Value);
        selectCommand.Parameters.AddWithValue("@DateTo", postedToDate.Value);
    ...
    public void ExportToSpreadsheet(DataTable table, string FileNameWithPath)
    {
        ...
        //string FilenName = "attachment; filename=ProductivityHeadCountCandidateDetail.xls";
        //HttpContext.Current.Response.AddHeader("content-disposition", FilenName);
