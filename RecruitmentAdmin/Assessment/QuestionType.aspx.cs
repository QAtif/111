...
public partial class QuestionType : CustomBasePage//XRecBase
{
    ...
    protected void rptSection_ItemDataBound(object source, RepeaterItemEventArgs e)
    {
        ...
        try
        {
            ...
            HiddenField control2 = (HiddenField)e.Item.FindControl("hfQuestionTypeCode");
            ((HtmlControl)e.Item.FindControl("btnEdit")).Attributes.Add("href", "AddEditQuestionType.aspx?qtid=" + control2.Value);
        }
        catch (Exception ex)
        {
            LogError.Write(LogError.AppType.RecruitmentModule, ex.StackTrace, ex, UserCode.ToString());
    ...
    protected void rptSection_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        {
            connection.Open();
            SqlCommand selectCommand = new SqlCommand("XRec_Delete_QuestionType", connection);
            selectCommand.CommandType = CommandType.StoredProcedure;
            selectCommand.Parameters.AddWithValue("@QuestionTypeCode", e.CommandArgument);
    ...
    public void ShowData()
    {
        ...
        try
        {
            if (connection.State == ConnectionState.Closed)
                connection.Open();
            SqlCommand selectCommand = new SqlCommand("XRec_Select_QuestionType ", connection);
            selectCommand.CommandType = CommandType.StoredProcedure;
