...
public partial class Section : CustomBasePage, IRequiresSessionState
{
    #region Variables
    ...
    protected void rptSection_ItemDataBound(object source, RepeaterItemEventArgs e)
    {
        ...
        try
        {
            ...
            HiddenField control2 = (HiddenField)e.Item.FindControl("hfSectionCode");
            ((HtmlControl)e.Item.FindControl("btnEdit")).Attributes.Add("href", "AddEditSection.aspx?sid=" + control2.Value);
        }
        catch (Exception ex)
        {
            LogError.Write(LogError.AppType.RecruitmentModule, ex.StackTrace, ex, UserCode.ToString());
    ...
    protected void rptSection_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        {
            connection.Open();
            SqlCommand selectCommand = new SqlCommand("XRec_Delete_Section", connection);
            selectCommand.CommandType = CommandType.StoredProcedure;
            selectCommand.Parameters.AddWithValue("@SectionCode", e.CommandArgument);
    ...
    public void ShowData()
    {
        ...
        try
        {
            if (connection.State == ConnectionState.Closed)
                connection.Open();
            SqlCommand selectCommand = new SqlCommand("XRec_Select_Section ", connection);
            selectCommand.CommandType = CommandType.StoredProcedure;
