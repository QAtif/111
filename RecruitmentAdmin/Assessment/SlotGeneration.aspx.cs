...
public partial class SlotGeneration : CustomBasePage//XRecBase
{
    #region Variables
    ...
    public void GetDetails()
    {
        ...
        if (dataTable.Rows.Count > 0)
        {
            SlotGeneration.objDV = dataTable.DefaultView;
            PageNo = 0;
            trPagingControls.Attributes.Add("style", "display:block");
            lblMsg.Visible = false;
            ltrMsg.Visible = false;
            rptSlot.DataSource = ApplyPaging(SlotGeneration.objDV, PageNo);
            rptSlot.DataBind();
        }
        else
        {
    ...
    public PagedDataSource ApplyPaging(DataView DV, int PageNo)
    {
        ...
        objPDS.PageSize = SlotGeneration.PgSize;
