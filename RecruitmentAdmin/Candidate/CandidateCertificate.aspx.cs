...
public partial class CandidateCertificate : CustomBasePage
{
    ...
    private void BindData()
    {
        ...
        using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["RecruitmentAdminConn"].ToString()))
        {
            ...
            using (SqlCommand selectCommand = new SqlCommand("XRec_SelectCandidateCertificate", connection))
            {
                ...
                selectCommand.Parameters.AddWithValue("@CandidateCode", CandidateCertificate.CID);
    ...
    private void UpdateCandidateQulification()
    {
        using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["RecruitmentAdminConn"].ToString()))
        {
            ...
            using (SqlCommand sqlCommand = new SqlCommand("XRec_UpdateCandidateQualificationAdmin", connection))
            {
                ...
                sqlCommand.Parameters.AddWithValue("@CandidateCode", Convert.ToInt32(CandidateCertificate.CID));
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (secQueryString["CID"] != null)
            CandidateCertificate.CID = secQueryString["CID"].ToString();
        ShowHidePosition();
        try
        {
            if (IsPostBack)
        ...
        {
            LogError.Write(LogError.AppType.Candidate, ex.Message + " " + ex.StackTrace, ex, CandidateCertificate.CID);
        }
    }
        ...
        try
        {
                ...
                using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["RecruitmentAdminConn"].ToString()))
                {
                    ...
                    try
                    {
                        using (SqlCommand sqlCommand = new SqlCommand("XRec_DeactivateCandidateQualification", connection))
                        {
                            ...
                            sqlCommand.Parameters.AddWithValue("@UpdatedBy", Convert.ToInt32(CandidateCertificate.CID));
            ...
            using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["RecruitmentAdminConn"].ToString()))
            {
                ...
                try
                {
                    ...
                    {
                        selectCommand.CommandType = CommandType.StoredProcedure;
                        selectCommand.Parameters.AddWithValue("@CandidateCode", Convert.ToInt32(CandidateCertificate.CID));
