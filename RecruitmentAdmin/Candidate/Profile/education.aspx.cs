...
public partial class Candidate_Profile_education : CustomBasePage, IRequiresSessionState
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (secQueryString["CID"] == null)
            return;
        Candidate_Profile_education.CID = secQueryString["CID"].ToString();
        try
        {
            if (IsPostBack)
                return;
        ...
        {
            LogError.Write(LogError.AppType.RecruitmentAdmin, ex.Message + " " + ex.StackTrace, ex, Candidate_Profile_education.CID);
        }
    }
    ...
    private void BindData()
    {
        ...
        using (SqlCommand selectCommand = new SqlCommand("XRec_SelectCandidateEducationDetails", connection))
        {
            ...
            selectCommand.Parameters.AddWithValue("@CandidateCode", Convert.ToInt32(Candidate_Profile_education.CID));
    ...
    protected void GetNotValidDetails(int CQC)
    {
        ...
        try
        {
            ...
            chkEducationStatus.Checked = dataSet.Tables[0].Rows[0]["CurrentlyStuding"].ToString() == "2" || dataSet.Tables[0].Rows[0]["CurrentlyStuding"].ToString() == "0";
        ...
        {
            LogError.Write(LogError.AppType.RecruitmentAdmin, ex.StackTrace, ex, Candidate_Profile_education.CID);
        }
    }
    ...
    {
        DataSet dataSet = new DataSet();
        SqlDataAdapter sqlDataAdapter = new SqlDataAdapter("XRec_SelectLevelOfEducation", new SqlConnection(ConfigurationManager.ConnectionStrings["RecruitmentAdminConn"].ConnectionString));
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            StatusManagement.MarkProfileStatus(connection, Convert.ToInt32(Candidate_Profile_education.CID), Constants.ModuleCode.ProfileStatus, Constants.CandidateProfileStatus.FilledEducationalQualificationDiplomaPending, Request.UserHostAddress, UserCode, Constants.ProfileNavigation.EducationalQualification);
            BindData();
    ...
    private void UpdateCandidateEduaction()
    {
        ...
        {
            connection.Open();
            SqlCommand selectCommand = new SqlCommand("XRec_UpdateCandidateQualificationDetailsAdmin", connection);
            selectCommand.CommandType = CommandType.StoredProcedure;
            selectCommand.Parameters.AddWithValue("@CandidateCode", Convert.ToInt32(Candidate_Profile_education.CID));
            ...
            selectCommand.Parameters.AddWithValue("@EducationStatus", (chkEducationStatus.Checked ? 2 : 1));
            ...
            selectCommand.Parameters.AddWithValue("@GPA", txtGPA.Text == string.Empty ? "0.00" : txtGPA.Text);
            selectCommand.Parameters.AddWithValue("@Percentage", txtPercentage.Text == string.Empty ? "0.00" : txtPercentage.Text);
            ...
            selectCommand.Parameters.AddWithValue("@Grade", txtGrade.Text);
            ...
            sqlCommand1.Parameters.AddWithValue("@CandidateCode", Convert.ToInt32(Candidate_Profile_education.CID));
            ...
            for (int index = 0; index <= strArray1.Length - 1; ++index)
            {
                ...
                sqlCommand2.Parameters.AddWithValue("@CandidateCode", Convert.ToInt32(Candidate_Profile_education.CID));
        ...
        {
            LogError.Write(LogError.AppType.RecruitmentAdmin, ex.StackTrace, ex, Session["CC"].ToString());
        }
        finally
        {
            if (connection.State == ConnectionState.Open)
        ...
        if (e.CommandName == "Delete" || e.CommandName == "Delete")
        {
            if (e.CommandName == "Delete")
            {
                try
                {
                    ...
                    sqlCommand.Parameters.AddWithValue("@CandidateCode", Convert.ToInt32(Candidate_Profile_education.CID));
                    sqlCommand.ExecuteNonQuery();
                    BindData();
                    if (lvEducation.Items.Count <= 0)
                    {
                        StatusManagement.RemoveProfileStatus(connection, Convert.ToInt32(Candidate_Profile_education.CID), Constants.ModuleCode.ProfileStatus, Request.UserHostAddress, UserCode, Constants.ProfileNavigation.EducationalQualification);
    ...
    private void LoadCandidateMajors(int CandidateQualification_Code)
    {
        try
        {
            ...
            for (int index = 0; index < dataSet.Tables[0].Rows.Count; ++index)
            {
                ...
                Candidate_Profile_education profileEducation = this;
