...
public partial class Candidate_StatusSummaryDetail : System.Web.UI.Page
{
    ...
    public int AllCurrentPage
    {
        ...
        set
        {
            ViewState["_CurrentPage"] = (object)value;
        }
    }
        ...
        using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["RecruitmentAdminConn"].ToString()))
        {
            ...
            try
            {
                using (SqlCommand selectCommand = new SqlCommand("XRec_SelectCandidateDetails", connection))
                {
                    ...
                    selectCommand.Parameters.AddWithValue("@Category_Code", (object)Convert.ToInt32(Request.QueryString["Cat"].ToString()));
                    selectCommand.Parameters.AddWithValue("@StatusCode_Code", (object)Convert.ToInt32(Request.QueryString["Status"].ToString()));
                ...
                if (dataSet != null && dataSet.Tables != null)
                {
                    ...
                    if (dataSet.Tables[0].Rows.Count > 0)
                    {
                        ...
                        rptCandidateLists.DataSource = (object)pagedDataSource;
                        rptCandidateLists.DataBind();
                    }
                    else
                    {
                        lblMsg.Text = "No record found.";
                        rptCandidateLists.DataSource = (object)null;
                        rptCandidateLists.DataBind();
