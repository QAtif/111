...
public partial class Profile_ShortListCandidates__BK : CustomBasePage, IRequiresSessionState
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            Profile_ShortListCandidates__BK.PageSize = 25;
            connection.Open();
    ...
    protected void rptProfileLists_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType != ListItemType.AlternatingItem && e.Item.ItemType != ListItemType.Item)
            return;
        ((Label)e.Item.FindControl("lblSno")).Text = Convert.ToString(PageNo * Profile_ShortListCandidates__BK.PageSize + (e.Item.ItemIndex + 1));
    ...
    public void GetRequisitionDetail(ref SqlConnection connection)
    {
        ...
        if (dataSet.Tables[0].Rows.Count <= 0)
            return;
        Profile_ShortListCandidates__BK.objDV = dataSet.Tables[0].DefaultView;
        trPagingControls.Attributes.Add("style", "display:''");
        PageNo = 0;
        rptProfileLists.DataSource = ApplyPaging(Profile_ShortListCandidates__BK.objDV, PageNo);
    ...
    public PagedDataSource ApplyPaging(DataView DV, int PageNo)
    {
        ...
        objPDS.PageSize = Profile_ShortListCandidates__BK.PageSize;
