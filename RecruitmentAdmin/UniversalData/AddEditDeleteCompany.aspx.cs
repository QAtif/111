...
public partial class UniversalData_AddEditDeleteCompany : CustomBasePage
{
    #region Variables
    SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["RecruitmentAdminConn"].ConnectionString);
    SqlConnection Errlogconnection = new SqlConnection(ConfigurationManager.ConnectionStrings["errLogConnection"].ToString());
    int addCompanyCount = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            if (string.IsNullOrEmpty(counter.Value))
                counter.Value = addCompanyCount.ToString();
            UniversalData_AddEditDeleteCompany.PageSize = 25;
            connection.Open();
    ...
    protected void rptCompany_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        try
        {
            ((Label)e.Item.FindControl("lblSno")).Text = Convert.ToString(PageNo * UniversalData_AddEditDeleteCompany.PageSize + (e.Item.ItemIndex + 1));
            Label control1 = (Label)e.Item.FindControl("lblCompanyName");
            TextBox control2 = (TextBox)e.Item.FindControl("txtCompanyName");
            ...
            if (Convert.ToInt32(DataBinder.Eval(e.Item.DataItem, "Company_Code")) == 0)
            {
    ...
    protected void rptCompany_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName == "Save")
            {
                if (control2.Text != "")
                {
                    SqlCommand sqlCommand = new SqlCommand("XRec_updateCompany", connection);
                    sqlCommand.Parameters.AddWithValue("@CompanyID", e.CommandArgument.ToString());
                    sqlCommand.Parameters.AddWithValue("@CompanyName", control2.Text);
            ...
            if (!(e.CommandName == "Delete"))
                return;
            SqlCommand sqlCommand1 = new SqlCommand("XRec_DeleteCompany", connection);
            sqlCommand1.Parameters.AddWithValue("@CompanyID", e.CommandArgument.ToString());
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        {
            connection.Open();
            foreach (RepeaterItem repeaterItem in rptCompany.Items)
            {
                TextBox control1 = (TextBox)repeaterItem.FindControl("txtCompanyName");
                HiddenField control2 = (HiddenField)repeaterItem.FindControl("hdnCompanyCode");
                if (control1.Text != "")
                {
                    SqlCommand sqlCommand = new SqlCommand("XRec_updateCompany", connection);
                    sqlCommand.Parameters.AddWithValue("@CompanyID", control2.Value);
                    sqlCommand.Parameters.AddWithValue("@CompanyName", control1.Text);
                    sqlCommand.Parameters.AddWithValue("@Updated_By", UserCode);
                    sqlCommand.Parameters.AddWithValue("@Updated_IP", Request.UserHostAddress.ToString());
                    sqlCommand.CommandType = CommandType.StoredProcedure;
                    Convert.ToString(sqlCommand.ExecuteScalar());
    ...
    protected void lnkSearch_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            SqlCommand selectCommand = new SqlCommand("XRec_SelectAllCompany", connection);
            selectCommand.Parameters.AddWithValue("@CompanyName", txtCompanyName.Text.Trim());
            ...
            else
            {
                ...
                arr.Add("0");
                arr.Add("");
                arr.Add(addCompanyCount);
                AddDataToTable(arr, (DataTable)ViewState["myDatatable"], true);
            }
            UniversalData_AddEditDeleteCompany.objDV = ((DataTable)ViewState["myDatatable"]).DefaultView;
            trPagingControls.Attributes.Add("style", "display:''");
            PageNo = 0;
            rptCompany.DataSource = ApplyPaging(UniversalData_AddEditDeleteCompany.objDV, PageNo);
            rptCompany.DataBind();
        }
        catch (Exception ex)
        {
    ...
    private void AddNewCompany()
    {
        ...
        rptCompany.DataSource = ((DataTable)ViewState["myDatatable"]).DefaultView;
        rptCompany.DataBind();
    }
    ...
    private void SaveData()
    {
        ...
        {
            ArrayList arr = new ArrayList();
            Label control1 = repeaterItem.FindControl("lblCompanyName") as Label;
            repeaterItem.FindControl("txtCompanyName");
            HiddenField control2 = repeaterItem.FindControl("hdnCompanyCode") as HiddenField;
    ...
    public PagedDataSource ApplyPaging(DataView DV, int PageNo)
    {
        ...
        objPDS.PageSize = UniversalData_AddEditDeleteCompany.PageSize;
