function Filter_On_PageLoad(term, _id, cellNr) {
    ...
    for (var r = 1; r < table.rows.length; r++) {
        if (table.rows[r].cells[cellNr] != null) {
            ele = table.rows[r].cells[cellNr].innerHTML.replace(/<[^>]+>/g, "");
            if (table.rows[r].id.indexOf('trFoodType') != -1) {
...
/*
* A very simple script to filter a table on Page Load
*
...
function FilterRows(term, _id, cellNr) {
