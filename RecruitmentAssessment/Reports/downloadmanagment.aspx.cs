...
public partial class Reports_downloadmanagment : Page, IRequiresSessionState
{
    ...
    public static void ZipFiles(string inputFolderPath, string outputPathAndFile, string password)
    {
        ...
        try
        {
            ArrayList fileList = Reports_downloadmanagment.GenerateFileList(inputFolderPath);
        ...
        {
            LogError.Write(AppType.MHS, ex.Message + path1 + " " + ex.StackTrace, ex);
        }
    }
    ...
    private static ArrayList GenerateFileList(string Dir)
    {
        ...
        {
            foreach (object file in Reports_downloadmanagment.GenerateFileList(directory))
                arrayList.Add(file);
        }
        return arrayList;
    }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (chkFolder.Checked)
            {
                ...
                {
                    Reports_downloadmanagment.ZipFiles(Server.MapPath(txtfilepath.Text), outputPathAndFile, "");
                    if (File.Exists(Server.MapPath(txtfilepath.Text + outputPathAndFile)))
                    {
                        ...
                        {
                            LogError.Write(AppType.MHS, ex.Message + " " + ex.StackTrace, ex);
                        }
                        finally
                        {
                            binaryReader.Close();
