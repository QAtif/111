    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (ddlQuestionType.SelectedValue == "4")
        {
            if (fuDocument.HasFile)
            {
                ...
                file = ConfigurationManager.AppSettings["TestDocuments"].ToString() + file;

                if (!System.IO.Directory.Exists(Server.MapPath(ConfigurationManager.AppSettings["TestDocuments"].ToString())))
                {
                    System.IO.Directory.CreateDirectory(Server.MapPath(ConfigurationManager.AppSettings["TestDocuments"].ToString()));
                }
        ...
        {
            connection.Open();
            SqlCommand sqlCommand = new SqlCommand("XREC_Insert_Update_Question", connection);
            sqlCommand.CommandType = CommandType.StoredProcedure;
            //<!--</body-->
