...
public partial class QuestionType : XRecBase
{
    ...
    public void ShowData()
    {
        ...
        try
        {
            if (connection.State == ConnectionState.Closed)
                connection.Open();
            SqlCommand sqlCommand = new SqlCommand("XRec_Select_QuestionType ", connection);
    ...
    protected void rptSection_ItemDataBound(object source, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            HiddenField hfQuestionTypeCode = (HiddenField)e.Item.FindControl("hfQuestionTypeCode");
            HtmlAnchor btnEdit = (HtmlAnchor)e.Item.FindControl("btnEdit");

            string RedirectingLink = "AddEditQuestionType.aspx?" + "qtid" + "=" + hfQuestionTypeCode.Value;
    ...
    protected void rptSection_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Delete")
        {
            ...
            {
                connection.Open();
                SqlCommand sqlCommand = new SqlCommand("XRec_Delete_QuestionType", connection);
                sqlCommand.CommandType = CommandType.StoredProcedure;
                sqlCommand.Parameters.AddWithValue("@QuestionTypeCode", e.CommandArgument);
