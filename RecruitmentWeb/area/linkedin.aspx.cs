...
public partial class area_LinkedIn : CustomBaseClass
{
    #region Variables
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            string str = _oauth.AuthorizationLinkGet();
            Application["reuqestToken"] = _oauth.Token;
            Application["reuqestTokenSecret"] = _oauth.TokenSecret;
            Application["oauthLink"] = str;
            hdnRequestToken.Value = _oauth.Token;
            hdnTokenSecret.Value = _oauth.TokenSecret;
            hypAuthToken.NavigateUrl = "javascript:;";
            hypAuthToken.Attributes.Add("onclick", "window.open('" + str + "')");
            int num = str.IndexOf('=');
            hdnAuth_token.Value = str.Substring(num + 1, str.Length - (num + 1));
            tblinkedin.Attributes.Add("style", "display:inline");
        }
        catch (Exception ex)
        {
